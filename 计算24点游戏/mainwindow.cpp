...
void MainWindow::newTurn()
{
    ...
    do{
        fourNums.clear();
        for (int n=0; n<4; ++n)
            fourNums.push_back(dis(engine));
    }while(generate24(fourNums)!=true);

    ui->label_1->setText(QString().setNum(fourNums[0]));
    ui->label_2->setText(QString().setNum(fourNums[1]));
    ui->label_3->setText(QString().setNum(fourNums[2]));
    ui->label_4->setText(QString().setNum(fourNums[3]));
...
{
    timer->stop();
    if(check(fourNums,ui->lineEdit->text()))
    {
